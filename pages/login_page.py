    ...
    if trigger_id == 'login-button' and login_clicks > 0:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                return [{'display': 'none'}] * 3 + [{'display': 'block'}] + ["Login", f"Errore: {res.status_code} - {res.text}"]     
        except requests.exceptions.RequestException as e:
            return [{'display': 'none'}] * 3 + [{'display': 'block'}] + ["Login", f"Errore di connessione: {str(e)}"]
    return [{'display': 'none'}] * 3 + [{'display': 'block'}] + ["Login", ""]

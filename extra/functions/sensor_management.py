...
# Funzione per importare un campione di dati recenti dall'API e inserirli nel database
def sensor_import_sample_data(n=100):
    """
    Importa un campione di n dati più recenti dall'API dei sensori e li salva nel database.
    """
    print(f"Importazione di un campione di {n} dati sensori recenti...")
    
    conn = get_db_connection()
    sensor_create_table()  # Crea la tabella se non esiste

    # Scarica i dati più recenti
    data = sensor_get_filtered_data(limit=n)

    if data:
        sensor_insert_data(conn, data)
        print(f"Inseriti {len(data)} record sensori nel database.")
    else:
        print("Nessun dato da inserire. Verifica la connessione o l'API.")

    conn.close()


# Funzione per ottenere gli ultimi dati dal database per visualizzazione
def sensor_get_last_records(n=20):
    """
    Recupera gli ultimi n record dalla tabella sensori_aria.
    """
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
...
def sensor_manage_database():
    ...
    action = input("Scegli un'azione (creare/eliminare/eliminare_n/importa_sample): ").strip().lower()
    ...
    elif action.startswith("importa_sample"):
        try:
            n = int(action.split('_')[-1]) if len(action.split('_')) > 1 else 100
            print(f"Importazione di un sample di {n} dati sensori...")
            sensor_import_sample_data(n)
        except ValueError:
            print("Input non valido. Inserisci un numero valido per il sample da importare.")

    else:
        print("Azione non valida. Inserisci 'creare', 'eliminare', 'eliminare_n' o 'importa_sample'.")
